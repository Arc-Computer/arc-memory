...
import { LLMClientFactory } from "./llm/llm-client-factory.js";
import { CommentFormatter } from "./llm/comment-formatter.js";

/**
 * Interface for a diff snippet
 */
 ...
 */
function extractRelevantDiffs(changedFiles: any[], insights: any): DiffSnippet[] {
  ...
  app.on(["pull_request.opened", "pull_request.synchronize"], async (context) => {
    ...
    try {
      ...
      try {
        ...
        // Check if LLM is enabled in the configuration
        if (config.features.useLLM) {
          try {
            ...
            const prContextProcessor = new PRContextProcessor(llmClient, app.log);

            // Generate insights using the LLM
            app.log.info(`Generating LLM insights for PR #${pr.number}`);
            ...
            const diffSnippets = extractRelevantDiffs(prContext.changedFiles, insights);

            // Add code diff visualizations if enabled
            let finalComment = comment;
            if (config.comment.includeCodeSnippets && diffSnippets.length > 0) {
              finalComment += '\n\n## Code Changes\n\n';
            ...
            const llmComment = context.issue({
              body: finalComment,
            });

            await context.octokit.issues.createComment(llmComment);
            app.log.info(`Added LLM-generated comment to PR #${pr.number}`);
          } catch (llmError: any) {
            app.log.error(`Error generating LLM insights: ${llmError}`);

            // Fall back to the traditional comment if LLM fails
            const detailedComment = context.issue({
              body: generateCommentBody(config, {
                ...
                commits,
                relatedPRs,
              }) + `\n\n> Note: LLM-powered insights could not be generated due to an error: ${llmError.message || String(llmError)}`,
            });

            await context.octokit.issues.createComment(detailedComment);
            app.log.info(`Added fallback detailed comment to PR #${pr.number}`);
          }
        } else {
          // Generate a traditional detailed comment with the context
          const detailedComment = context.issue({
            body: generateCommentBody(config, {
              linearTickets,
              adrs,
